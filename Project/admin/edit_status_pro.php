...
if (!$connect) {
    echo "<script>alert('ไม่สามารถเชื่อมต่อฐานข้อมูล'); window.location.href = 'manage_status.php';</script>";
    exit();
}

// ตรวจสอบว่ามีค่า request_id
$request_id = isset($_POST['request_id']) ? $_POST['request_id'] : null;
if (!$request_id) {
    echo "<script>alert('ไม่พบข้อมูลการร้องขอ'); window.location.href = 'manage_status.php';</script>";
    exit();
}

// รับค่าจากฟอร์มและทำการป้องกัน SQL Injection
// รับค่าจากฟอร์มและทำการป้องกัน SQL Injection
$approvalStatus = mysqli_real_escape_string($connect, $_POST['status']);  // สถานะการอนุมัติ
$carId = isset($_POST['car_id']) ? mysqli_real_escape_string($connect, $_POST['car_id']) : null;  // id ของรถยนต์ (ถ้ามี)
$driverId = isset($_POST['driver_id']) ? mysqli_real_escape_string($connect, $_POST['driver_id']) : null;  // id ของพนักงานขับรถ (ถ้ามี)
$notes = isset($_POST['notes']) ? mysqli_real_escape_string($connect, $_POST['notes']) : '';  // หมายเหตุ (ถ้าไม่มีให้เป็น '')
...
if ($notes !== '') {
    ...
    // หากไม่มีหมายเหตุให้ใส่ค่าว่างในฐานข้อมูล
    $sql1 = "INSERT INTO `notes`(`request_id`, `note_text`) VALUES ('$request_id', '')";
    if (!mysqli_query($connect, $sql1)) {
        echo "<script>alert('เกิดข้อผิดพลาดในการบันทึกหมายเหตุ: " . mysqli_error($connect) . "'); window.location.href = 'manage_status.php';</script>";
        exit();
    }
